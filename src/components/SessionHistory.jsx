...
export default function SessionHistory({ sessions }) {
  ...
  if (sessions.length === 0) {
    return (
      <div>
        <h3 className="text-center mb-6 text-2xl font-bold text-gray-800">{t('sessionHistory')}</h3>
        <div className="text-center py-8">
          <p className="text-gray-600">{t('noSessions')}</p>
          <p className="text-gray-600">{t('startFirstSession')}</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="text-center bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-2xl border-2 border-yellow-200 shadow-md">
          <div className="text-sm text-gray-600 mb-2 font-semibold">{t('bestScore')}</div>
        ...
        <div className="text-center bg-gradient-to-br from-orange-50 to-yellow-50 p-4 rounded-2xl border-2 border-orange-200 shadow-md">
          <div className="text-sm text-gray-600 mb-2 font-semibold">{t('averageScore')}</div>
          <div className="text-2xl font-bold text-orange-500">{averageScore}</div>
        </div>
        <div className="text-center bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-2xl border-2 border-yellow-200 shadow-md">
          <div className="text-sm text-gray-600 mb-2 font-semibold">{t('sessionCount')}</div>
          <div className="text-2xl font-bold text-orange-500">{sessions.length}</div>
        </div>
      </div>

      <div>
        <h4 className="mb-4 text-center text-lg font-semibold text-gray-800">{t('recentSessions')}</h4>
      ...
      {sessions.length > 5 && (
        <div className="text-center mt-4 text-gray-600 italic">
          <p>+{sessions.length - 5} {t('moreSessions')}</p>
        </div>
      )}
    </div>
  );
